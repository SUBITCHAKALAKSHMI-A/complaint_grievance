...
const insertDefaultData = () => {
  ...
  categories.forEach((category, index) => {
    db.run('INSERT OR IGNORE INTO categories (name, description) VALUES (?, ?)', category, (err) => {
...
const insertDefaultUser = () => {
  ...
  db.run('INSERT OR IGNORE INTO users (email, password, full_name, role) VALUES (?, ?, ?, ?)', 
    ['admin@complaintportal.com', hashedPassword, 'System Administrator', 'super_admin'], (err) => {
    if (err) console.error('User insert error:', err);
    insertDefaultEscalationRules();
  });
};

const insertDefaultEscalationRules = () => {
  ...
  escalationRules.forEach((rule, index) => {
    db.run('INSERT OR IGNORE INTO escalation_rules (category_id, priority, hours_before_escalation, escalate_to_role) VALUES (?, ?, ?, ?)', rule, (err) => {
      if (err) console.error('Escalation rule insert error:', err);
      if (index === escalationRules.length - 1) {
